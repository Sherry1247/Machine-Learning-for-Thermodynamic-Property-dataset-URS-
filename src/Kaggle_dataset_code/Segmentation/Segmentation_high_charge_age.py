...
file_name = 'high_charge_charges.csv'
X_VAR = 'age'
Y_VAR = 'charges'
...
# NOTE: The user provided an absolute path which is ignored here. We use the accessible file name.
df = pd.read_csv('/Users/daisiqi/Machine-Learning-for-Thermodynamic-Property-dataset-URS-/data/Kaggle_medical_insurance_dataset/high_charge_charges.csv')
...
# Calculate Pearson r on the cleaned data
pearson_r, _ = pearsonr(X.flatten(), y.flatten())
...
results_df['Pearson r'] = results_df['Pearson r'].round(4)
results_df = results_df.sort_values(by='R^2', ascending=False)
# Table printing is handled in the execution output
